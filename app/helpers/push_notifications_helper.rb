module PushNotificationsHelper
  ...
  def self.send_push(body)
    HTTParty.post 'https://onesignal.com/api/v1/notifications', headers: HEADERS, body: body, logger: @push_logger,
                                                                log_level: :debug, log_format: :curl
  end

  # Send push to all stores.
  def self.new_request_created(_replacement_request, url)
    push_body = @body.merge(
      {
        'included_segments' => ['locales_comerciales'],
        'url' => url,
        'data' => { "type": 'new_request_created' },
        'contents' => { 'en' => 'New request created! Add a proposal.',
                        'es' => '¡Nueva solicitud ingresada! Agregue una propuesta.' }
      }
    ).to_json
