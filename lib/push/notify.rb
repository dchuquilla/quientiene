 module Push
  class Notify
    ...
    def self.new_request_created(replacement_request)
      push_body = @body.merge(
        { 
          "included_segments" => ["locales_comerciales"],
          "url" => new_replacement_proposal_path(replacement_request_id: replacement_request.id),
    ...
    def self.new_proposal_created(replacement_proposal)
      push_body = @body.merge(
        { 
          "url" => replacement_proposals_path(replacement_request_id: replacement_proposal.replacement_request_id),
    ...
    # Send push to specific user.
    def self.proposal_accepted(replacement_proposal)
      push_body = @body.merge(
        { 
          "url" => replacement_proposals_path(replacement_request_id: replacement_proposal.replacement_request_id),
...
#Push::Notify.new_proposal_created(replacement_proposal)
#Push::Notify.proposal_accepted(replacement_proposal)

#Push::Notify.new_request_created(replacement_request)